...
# input: (1) format ('fasta' or 'pdb' or 'csv') - (2) list of files
def read(format,*files):
	...
	if format == 'fasta' or format == 'FASTA':
		...
		with open( files[0][0], 'rb') as csvfile:
...
def factor(s,action,type):

	# Actions:
	# - plot: shows the factor figure  
	# - save: save the factor figure in the disk
	s = s*(1.0/float(np.sum(s)))
	if type == 'svds' or type == 'SVDS':
		s = list(reversed(s))
